...
def main():
  ...
  for i in range(0,640,16):
  	y = int(math.sin(i / 640.0 *2* math.pi) * 50+400 )
  	x=int(math.cos(i / 640.0 *2* math.pi) * 400+400 )
  	plotPoints.append([x, y])
  angle=[]
  for i in range(-90,-360,-9):
  	angle.append(i)
  for i in range(0,-90,-9):
  	angle.append(i)
  #for i in range(30,len(plotPoints)-1):
  	#x=plotPoints[i+1][0]-plotPoints[i][0]
  	#y=plotPoints[i+1][1]-plotPoints[i][1]
  	#z=math.atan2(y,x)
  	#j=math.tan(z)*-180/math.pi
  	#angle.append(j)
  	#print(x,y,j)
  #angle.append(-90)
  ...
  while True:
    ...
    background3=pygame.transform.rotate(background2,angle[tu])
    ...
    screen.blit(background3,plotPoints[tu])
    #pygame.draw.lines(screen,[255,0,0],True,img2x,2)
    pygame.draw.aalines(screen,[255,0,0],True,plotPoints)
    print(img2x[0])
    pygame.display.update()
    time.sleep(0.1)
